...
function parcoursJSONNeg(jsonObj) {
  ...
  for (let i = 0; i < jsonObj.length; i++) {
    ...
    if (
      sentenceToConjug.text().includes("je") ||
      sentenceToConjug.text().includes("Je") ||
      sentenceToConjug.text().includes("J'") ||
      sentenceToConjug.text().includes("j'")
    ) {
      // Conjugaison à la première personne du singulier
      if (premPersSing !== null) {
        // Si le mots en question peut être conjugué et qu'il ne l'est pas déjà
        if (
          sentenceToConjug.text().includes(" " + name + " ") ||
          sentenceToConjug.text().includes("'" + name + " ")
        ) {
          ...
          if (phrase.length === 1) {
            ...
            if (vowel.includes(premPersSing.charAt(0))) {
              sentenceToConjug.text(
                sentenceToConjug
                  .text()
                  .replace(name, "n'" + premPersSing + " pas")
              ); // Alors remplace le par sa variante
            } else {
              sentenceToConjug.text(
                sentenceToConjug
                  .text()
                  .replace(name, "ne " + premPersSing + " pas")
              ); // Alors remplace le par sa variante
            }
          }
        }
      ...
      sentenceToConjug.text().includes("tu") ||
      sentenceToConjug.text().includes("Tu")
    ) {
          ...
          if (phrase.length === 1) {
            ...
            if (vowel.includes(deuxPersSing.charAt(0))) {
              sentenceToConjug.text(
                sentenceToConjug
                  .text()
                  .replace(name, "n'" + deuxPersSing + " pas")
              ); // Alors remplace le par sa variante
            } else {
              sentenceToConjug.text(
                sentenceToConjug
                  .text()
                  .replace(name, "ne " + deuxPersSing + " pas")
              ); // Alors remplace le par sa variante
            }
          }
        }
      ...
      sentenceToConjug.text().includes("nous") ||
      sentenceToConjug.text().includes("Nous")
    ) {
          ...
          if (phrase.length === 1) {
            ...
            if (vowel.includes(premPersPlur.charAt(0))) {
              sentenceToConjug.text(
                sentenceToConjug
                  .text()
                  .replace(name, "n'" + premPersPlur + " pas")
              ); // Alors remplace le par sa variante
            } else {
              sentenceToConjug.text(
                sentenceToConjug
                  .text()
                  .replace(name, "ne " + premPersPlur + " pas")
              ); // Alors remplace le par sa variante
            }
          }
        }
      ...
      sentenceToConjug.text().includes("vous") ||
      sentenceToConjug.text().includes("Vous")
    ) {
          ...
          if (phrase.length === 1) {
            ...
            if (vowel.includes(deuxPersPlur.charAt(0))) {
              sentenceToConjug.text(
                sentenceToConjug
                  .text()
                  .replace(name, "n'" + deuxPersPlur + " pas")
              ); // Alors remplace le par sa variante
            } else {
              sentenceToConjug.text(
                sentenceToConjug
                  .text()
                  .replace(name, "ne " + deuxPersPlur + " pas")
              ); // Alors remplace le par sa variante
            }
          }
        }
      ...
      sentenceToConjug.text().includes("eux") ||
      sentenceToConjug.text().includes("Eux") ||
      sentenceToConjug.text().includes("ils") ||
      sentenceToConjug.text().includes("Ils") ||
      sentenceToConjug.text().includes("elles") ||
      sentenceToConjug.text().includes("Elles")
    ) {
          ...
          if (phrase.length === 1) {
            ...
            if (vowel.includes(troisPersPlur.charAt(0))) {
              sentenceToConjug.text(
                sentenceToConjug
                  .text()
                  .replace(name, "n'" + troisPersPlur + " pas")
              ); // Alors remplace le par sa variante
            } else {
              sentenceToConjug.text(
                sentenceToConjug
                  .text()
                  .replace(name, "ne " + troisPersPlur + " pas")
              ); // Alors remplace le par sa variante
            }
          }
        }
          ...
          if (phrase.length === 1) {
            ...
            if (vowel.includes(troisPersSing.charAt(0))) {
              sentenceToConjug.text(
                sentenceToConjug
                  .text()
                  .replace(name, "n'" + troisPersSing + " pas")
              ); // Alors remplace le par sa variante
            } else {
              sentenceToConjug.text(
                sentenceToConjug
                  .text()
                  .replace(name, "ne " + troisPersSing + " pas")
              ); // Alors remplace le par sa variante
            }
          }
        }
